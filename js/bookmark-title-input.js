class BookmarkTitleInputElement extends HTMLInputElement {
  constructor() {
    super();
    this.datalist = null;
  }
  ...
  async fetchSuggestions(query) {
    try {
      if (query.length > 2) {
        ...
        const response = await fetch(apiUrl+`/bookmarks/index.php?title=${encodeURIComponent(query)}`);
  ...
  populateList(suggestions) {
    ...
    suggestions.forEach(suggestion => {
     	...
     	option.text = suggestion.title+' ('+suggestion.topic.title+')';
