...
class Topic implements CollectibleElement {
	...
	/**
	 * Le poids de la catégorie (exprimé en nombre de ressources incluses);
	 *
	 * @var int
	 * @since 2008-10-31
	 */
	...
	/**
	 * Obtient le nombre de catégories parentes
	 *
	 * @return int
	 * @since 01/04/2010
	 * @version 07/06/2014
	...
	/**
	 * Indique si la catégorie est une sous-catégorie
	 *
	 * @return bool
	 * @since 25/04/2010
	 */
		...
		try {
			...
			if ($topic->pushIntervalHigherLimit ( $this->getIntervalSize () ) + 1) { // on a fait de la place dans la rubrique de destination
				...
				if ($this->shiftInterval ( $shift )) { // on a décalé l'intervalle de la rubrique à transférer
					...
					throw new Exception ( 'échec du décalage de l\'intervalle de la catégorie à transférer' );
				}
			} else {
				throw new Exception ( 'échec de l\'extension de l\'intervalle de la catégorie cible' );
			}
		} catch ( Exception $e ) {
			$system->reportException ( __METHOD__, $e );
		...
		try {
			...
			if ($this->toDB ()) {
				if ($topic->pushIntervalHigherLimit ( 2 )) {
					...
					$i = new Interval ( ($l - 2), ($l - 1) );
					return $this->saveInterval ( $i );
				}
				throw new Exception ( 'L\'intervalle n\'a pu être agrandi' );
			}
			throw new Exception ( 'La nouvelle catégorie n\'a pu enregistrée' );
		} catch ( Exception $e ) {
			$system->reportException ( __METHOD__, $e );
			return false;
		}
	...
	/**
	 * Obtient les raccourcis vers d'autres catégories associés à cette catégorie.
	 *
	 * @since 26/02/2010
	 */
	public function getRelatedTopics() {
		...
		try {
			if (isset ( $this->relatedtopics )) {
				...
				// pour titres des catégories parentes, nécessaire échappement des caractères spéciaux CSV
	...
	/**
	 * Crée une relation mutuelle (raccourcis) avec la catégorie passée en paramètre.
	 *
	 * @param Topic $topic        	
	 * @since 24/01/2011
	 */
	...
	/**
	 * Supprime la relation mutuelle avec la catégorie passée en paramètre.
	 *
	 * @param Topic $topic        	
	 * @since 24/01/2011
	 */
	...
	/**
	 * Indique si la catégorie possède un raccourci vers la catégorie passée en paramètre.
	 *
	...
	public function spreadPrivacyToDescendants() {
		...
		try {
			if (is_null ( $this->getIntervalSize () )) {
				throw Exception ( 'Impossible de connaître l\'intervalle de la catégorie ' . $this->getId () );
			}
			$where = array ();
			$where [] = 'topic_interval_lowerlimit > :lowerlimit';
			$where [] = 'topic_interval_higherlimit < :higherlimit';
	...
	/**
	 * Transfère les sous-catégories de la catégorie dans une autre catégorie
	 *
	 * @param Topic $topic        	
	 * @return bool
	 * @version 04/10/2014
	...
	/**
	 * Obtient le nombre de ressources regroupées dans la catégorie.
	 *
	 * @return int
	 * @since 2009-01-29
	 * @version 07/06/2014
