...
class Bookmark implements CollectibleElement
{
    ...
    /**
     * Indique si la catégorie dans laquelle est attachée le signet est connue
     *
     * @since 06/08/2012
     */
    ...
    public function getHtmlLinkToInfo()
    {
        ...
        if ($system->isUserAuthenticated()) {
            $html = '<a href="' . $system->getProjectUrl() . '/bookmark_info.php?bookmark_id=' . $this->id . '">';
    ...
    public function getHtmlLinkToEdition($target = '_self', $type = 'icon')
    {
        ...
        switch ($type) {
            case 'icon':
                return '<a href="' . self::getEditionUrl($params) . '" target="' . $target . '"><i class="fa fa-pencil fa-lg"></i></a>';
            case 'text':
                return '<a href="' . self::getEditionUrl($params) . '" target="' . $target . '" >éditer</a>';
            case 'mixed':
                return '<a href="' . self::getEditionUrl($params) . '" target="' . $target . '"><i class="fa fa-pencil fa-lg"></i>&nbsp;éditer</a>';
        }
    }
    ...
    public function getHtmlLinkToPassword($target = '_self', $type = 'icon')
    {
        if ($this->login) {
            switch ($type) {
                case 'icon':
                    return '<a href="' . self::getPasswordUrl() . '" target="' . $target . '" ><i class="fa fa-key fa-lg"></i></a>';
                case 'text':
                    return '<a href="' . self::getPasswordUrl() . '" target="' . $target . '" >identifiants</a>';
                case 'mixed':
                    return '<a href="' . self::getPasswordUrl() . '" target="' . $target . '" ><i class="fa fa-key fa-lg"></i>&nbsp;identifiants</a>';
            }
        }
    }
