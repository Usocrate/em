...
class System
{
    ...
    public function getUsers()
    {
        try {
            $users = array();
            $data = $this->getPdo()
                ->query('SELECT * FROM ' . User::getTableName() . ' AS u')
                ->fetchAll();
            foreach ($data as $d) {
                $u = new User();
                $u->hydrate($d);
     ...
     * @since 15/08/2012
     */
    private function initBookmarkHitStats()
    {
        try {
     ...
     * @param
     *            $uploadedfile
     * @param $targettopic Rubrique
     *            où seront enregistrées les nouvelles ressources
     * @version 30/05/2008
     */
